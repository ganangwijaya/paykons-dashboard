...
const TransactionPage = () => {
  ...
  if (table.getRowModel().rows.length > 0) {
    totalData = table.getRowModel().rows.length;
    table.getRowModel().rows.map(d => {
    ...
    <Stack mt={4} gap={2}>
      <Grid templateColumns={'repeat(4, 1fr)'} gap={4}>
        <GridItem colSpan={{ base: 4, md: 2, lg: 1 }} >
          <Flex p={4} bg={bg} rounded={'xl'} gap={4} alignItems={'center'} pos={'relative'}>
            ...
            <Box pos={'absolute'} top={1} right={1}>
              <Menu closeOnSelect={false}>
                <MenuButton as={IconButton} variant={'ghost'} w={5} h={5} size={'xs'} fontSize={12} icon={<i className="ri-more-2-fill"></i>} />
                <MenuList fontSize={'xs'}>
                  <MenuOptionGroup textTransform={'capitalize'} fontSize={12} defaultValue='month' title='Filter By' type={'radio'}>
                    ...
                    <Box p={2} ml={2}>
                      <Filter column={table.getColumn('transactionDate')} table={table} />
                    </Box>
        ...
        <GridItem colSpan={{ base: 4, md: 2, lg: 1 }} >
          <Flex p={4} bg={bg} rounded={'xl'} gap={4} alignItems={'center'}>
            <Flex w={10} h={10} bg={iconBG} color={iconColor} justifyContent={'center'} alignItems={'center'} rounded={'full'} fontSize={'xl'}><i className="ri-file-list-3-fill"></i></Flex>
            <Box>
        ...
        <GridItem colSpan={{ base: 4, md: 2, lg: 1 }} >
          <Flex cursor={'pointer'} onClick={() => setColumnFilters([{ id: 'status', value: 'confirmed' }])} p={4} bg={bg} boxShadow={columnFilters.length > 0 ? columnFilters[0].value == 'confirmed' ? 'darkGreen' : 'green' : 'green'} _hover={{ boxShadow: 'darkGreen' }} transition={'all 0.3s ease-in-out'} rounded={'xl'} gap={4} alignItems={'center'}>
            <Flex w={10} h={10} bg={'green.500'} color={iconColor} justifyContent={'center'} alignItems={'center'} rounded={'full'} fontSize={'xl'}><i className="ri-checkbox-circle-fill"></i></Flex>
            <Box>
        ...
        <GridItem colSpan={{ base: 4, md: 2, lg: 1 }} >
          <Flex cursor={'pointer'} onClick={() => setColumnFilters([{ id: 'status', value: 'unconfirmed' }])} p={4} bg={bg} boxShadow={columnFilters.length > 0 ? columnFilters[0].value == 'unconfirmed' ? 'darkRed' : 'red' : 'red'} _hover={{ boxShadow: 'darkRed' }} transition={'all 0.3s ease-in-out'} rounded={'xl'} gap={4} alignItems={'center'}>
            <Flex w={10} h={10} bg={'red.500'} color={iconColor} justifyContent={'center'} alignItems={'center'} rounded={'full'} fontSize={'xl'}><i className="ri-spam-fill"></i></Flex>
            <Box>
